...
const float ENGINE_POWER = 250.0f;
...
void HandleUpdate(StringHash eventType, VariantMap& eventData)
{
    ...
    if (ui.focusElement is null)
    {
        ...
        vehicle.controls.Set(CTRL_BRAKE, input.keyDown[KEY_SPACE]);
...
class Vehicle : ScriptObject
{
    ...
    Node@ InitWheel(const String&in name, const Vector3&in offset)
    {
        ...
        wheelBody.friction = 60;
        wheelBody.mass = 1;
        wheelBody.linearDamping = 0.0f; // Some air resistance
    ...
    void FixedUpdate(float timeStep)
    {
        ...
        if (accelerator != 0.0f && accelerator > -1.0f)
        {
        ...
        } else if (accelerator <= -1.0f) {
            // Torques are applied in world space, so need to take the vehicle & wheel rotation into account
            Vector3 torqueVec = Vector3(0.0f, 0.0f, 0.0f);

            Vector3 vel = Vector3(0.0f, 0.0f, 0.0f);
            frontLeftBody.angularVelocity = vel;
            frontRightBody.angularVelocity = vel;
            rearLeftBody.angularVelocity = vel;
            rearRightBody.angularVelocity = vel;
